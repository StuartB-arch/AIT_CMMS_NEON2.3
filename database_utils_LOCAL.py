...
SIMPLIFIED VERSION FOR LOCAL POSTGRESQL
- Removed NEON-specific keepalive thread (not needed for local server)
- Removed connection max age tracking (not needed for local server)
- Reduced connect_timeout from 30s to 10s (local is fast)
- Simplified TCP keepalive settings (standard values)
- Kept all essential features: pooling, transactions, audit logging, user management
"""
...
class DatabaseConnectionPool:
    ...
    def initialize(self, db_config, min_conn=2, max_conn=20):
        """
        ...
        if self.pool is None:
            ...
            self.pool = psycopg2.pool.ThreadedConnectionPool(
                ...
                sslmode=db_config.get('sslmode', 'prefer'),  # 'prefer' for local, 'require' for remote
                # Standard TCP Keepalive settings for local server
                keepalives=1,              # Enable TCP keepalive
                keepalives_idle=60,        # Start keepalive after 60 seconds of idle
                keepalives_interval=10,    # Send keepalive every 10 seconds
                keepalives_count=5,        # Close connection after 5 failed keepalives
                # Connection timeout
                connect_timeout=10         # 10 second connection timeout (local server is fast)
            )
